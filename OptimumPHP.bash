...
if [ "$CENTOS" != "0" ] ; then
  yum -y install gcc gcc-c++ httpd libxml2-devel curl-devel openssl-devel libidn-devel libjpeg-devel libpng-devel freetype-devel libicu-devel libmcrypt-devel mysql-server mysql mysql-devel libxslt-devel autoconf libtool-ltdl-devel httpd-devel apr-devel apr subversion openldap-devel postgresql-devel ||
...
if [ "$UBUNTU" != "0" ] ; then
  ...
  if [ "$UVERSION" -ge 12 ] ; then
    if [ `getconf LONG_BIT` = "64" ] ; then      
      # Fix PHP configure script to find the LDAP libraries. There's no clean 
